    ...
    <div className="mx-auto max-w-7xl space-y-10 pb-12">
      <section className="relative overflow-hidden rounded-[2rem] border border-white/10 bg-[#111] min-h-[360px]">
        {featured?.cover_url ? (
          ...
          <div className="absolute inset-0 bg-[radial-gradient(circle_at_70%_20%,rgba(249,115,22,0.22),transparent_32%),linear-gradient(135deg,#161616,#050505)]" />
        )}
        ...
        <div className="relative flex min-h-[360px] max-w-2xl flex-col justify-end p-8 md:p-10">
          ...
          {featured ? (
            <Link href={`/learn/${featured.id}`} className="mt-6 inline-flex w-fit items-center gap-2 rounded-xl bg-[#f97316] px-5 py-3 text-sm font-black text-black transition hover:bg-[#fb923c]">
...
function Shelf({ title, products, icon }: { title: string; products: LearnProductRow[]; icon: ReactNode }) {
    ...
    <section>
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        {products.map(product => (
          <Link key={product.id} href={`/learn/${product.id}`} className="group overflow-hidden rounded-2xl border border-white/10 bg-[#111] transition hover:-translate-y-1 hover:border-[#f97316]/35">
            <div className="aspect-video bg-black">
              {product.cover_url ? (
                ...
                <div className="h-full w-full bg-[radial-gradient(circle_at_30%_20%,rgba(249,115,22,0.22),transparent_36%),linear-gradient(135deg,#181818,#080808)]" />
              )}
            </div>
            <div className="p-4">
              <p className="text-xs font-bold uppercase text-[#f97316]">{product.category || 'Flowyn'}</p>
